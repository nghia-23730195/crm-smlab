...
import { createStemModel, getNextStemCode } from "../actions";

export const runtime = "nodejs";

type NewStemModelPageProps = {
  searchParams: Promise<{
    error?: string;
  }>;
};

export default async function NewStemModelPage({
  searchParams,
}: NewStemModelPageProps) {
  const currentUser = await requireCurrentUser();
  const organizationId = currentUser.organizationId;
  const params = await searchParams;

  const nextCode = await getNextStemCode(organizationId);
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
              ...
              <span>/</span>
              <span className="text-slate-900">Thêm mô hình mới</span>
            </div>
            <h1 className="mt-1 text-xl font-bold text-slate-900">
              Thiết kế & Tạo mô hình / Kit học tập STEM mới
            </h1>
          </div>
        ...
        >
          <div className="border-b border-slate-100 pb-5 mb-6">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Khai báo tên mô hình, cấp học mục tiêu, linh kiện dự toán và tài liệu thiết kế mẫu thử.
            </p>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-400 mt-1 block">
                Tự động đánh mã liên tiếp: {nextCode}
              </span>
            </div>
            ...
            <div>
              ...
              <input
                ...
                defaultValue="Vật lý & Cơ kỹ thuật"
