...
import { createRdTopic, getNextOrderIndex } from "../actions";

export const runtime = "nodejs";

type NewRdTopicPageProps = {
  searchParams: Promise<{
    error?: string;
  }>;
};

export default async function NewRdTopicPage({ searchParams }: NewRdTopicPageProps) {
  const currentUser = await requireCurrentUser();
  const organizationId = currentUser.organizationId;
  const params = await searchParams;

  const nextOrderIndex = await getNextOrderIndex(organizationId);
  ...
  return (
    <div className="p-5 md:p-8">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
              ...
              <span>/</span>
              <span className="text-slate-900">Thêm đề tài mới</span>
            </div>
            <h1 className="mt-1 text-xl font-bold text-slate-900">
              Thêm đề tài nghiên cứu & phát triển mới
            </h1>
          </div>
        ...
        <form
          action={createRdTopic}
          className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-xs"
        >
          <div className="border-b border-slate-100 pb-5 mb-6">
            <h2 className="text-base font-bold text-slate-900">
              Thông tin đề tài tiềm năng
            </h2>
            <p className="mt-0.5 text-xs text-slate-500">
              Điền các thông tin chi tiết về đề tài, ý tưởng, đối tượng áp dụng và tài liệu tham khảo.
            </p>
          </div>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-400 mt-1 block">
                Tự động đánh số liên tiếp: {nextOrderIndex}
              </span>
            </div>
